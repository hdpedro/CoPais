...
- **OOM em uploads grandes (bug 15)**: `fetch().arrayBuffer()` lia o arquivo inteiro; `ImagePicker.fileSize=0` no Android furava o cap de 10MB → **crash nativo** (invisivel em `app_errors`). Fix = guard `upload-size.ts` + `FileSystem.getInfoAsync` antes do read, nos **4 uploads**. `documents` (#106) + **recibo/chat/foto** (#107, merged `f15d06c`). **✅ Entregue 2026-06-08 via OTA Android E iOS** (`ota:android` + `ota:ios`, `--platform` explícito; **10 runtimes cada** 1.0.5→1.0.21, `--branch production`; 1.0.5/1.0.6 re-incluídos manualmente a pedido do dono — `TARGET_VERSIONS` default segue excluindo-os). Esta OTA tambem **flushou o main** (~15 PRs desde a OTA de 04/jun, incl. **i18n nativo #103** — decisao do dono de publicar main inteiro; `TARGET_VERSIONS` += 1.0.21 ja estava). **Fix REAL futuro** (fora de escopo, maior impacto): streaming via `FileSystem.uploadAsync` + `supabase.storage.createSignedUploadUrl` elimina OOM em **qualquer** tamanho (hoje so barra >10MB; ≤10MB ainda lê na RAM).
