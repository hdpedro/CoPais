...
// `labelKey` resolvido no render via t() — nunca hardcodar PT aqui (vaza em EN).
const STATUS_META: Record<string, { bg: string; text: string; labelKey: string }> = {
  pending:        { bg: 'rgba(232,162,40,0.1)', text: '#E8A228', labelKey: 'expensesPage.statusPending' },
  approved:       { bg: 'rgba(76,175,80,0.1)',  text: '#4CAF50', labelKey: 'expensesPage.statusApproved' },
  rejected:       { bg: 'rgba(229,57,53,0.1)',  text: '#E53935', labelKey: 'expensesPage.statusRejected' },
  cancelled:      { bg: 'rgba(120,120,120,0.1)', text: '#666',   labelKey: 'expenses.status.cancelled' },
  cancel_pending: { bg: 'rgba(232,162,40,0.1)', text: '#E8A228', labelKey: 'expenses.status.cancelPending' },
};

const PRIORITY_META: Record<ExpensePriority, { bg: string; text: string; labelKey: string; rank: number }> = {
  info:      { bg: 'rgba(107,114,128,0.15)', text: '#4B5563', labelKey: 'collab.priorityInfo',      rank: 0 },
  important: { bg: 'rgba(245,158,11,0.18)',  text: '#B45309', labelKey: 'collab.priorityImportant', rank: 1 },
  urgent:    { bg: 'rgba(239,68,68,0.18)',   text: '#B91C1C', labelKey: 'collab.priorityUrgent',    rank: 2 },
};

/** Read-receipt timestamp, locale-aware. `t`/`intl` vêm do componente
 *  (não dá pra usar hooks aqui) — "agora" / "há Nmin" via i18n, hora/data via Intl. */
function formatReadAt(
  iso: string,
  t: (k: string, p?: Record<string, string | number>) => string,
  intl: { formatTime: (d: string) => string; formatDateShort: (d: string) => string },
): string {
  const d = new Date(iso);
  const now = new Date();
  const diffMin = Math.floor((now.getTime() - d.getTime()) / 60000);
  if (diffMin < 1) return t('notifications.justNow');
  if (diffMin < 60) return t('expenses.minutesAgo', { count: diffMin });
  const sameDay = d.toDateString() === now.toDateString();
  if (sameDay) return intl.formatTime(iso);
  return intl.formatDateShort(iso);
}

type StatusFilter = 'all' | 'pending' | 'approved' | 'rejected' | 'cancelled';

// i18n key suffixes — resolve at render with t(`expenses.audit.${action}`).
const ACTION_LABEL_KEYS: Record<string, string> = {
  created: 'expenses.audit.created', edited: 'expenses.audit.edited',
  approved: 'expenses.audit.approved', rejected: 'expenses.audit.rejected',
  cancel_requested: 'expenses.audit.cancelRequested', cancelled: 'expenses.audit.cancelled',
  reopened: 'expenses.audit.reopened', restored: 'expenses.audit.restored',
};
...
export default function DespesasScreen() {
  ...
  const sections = useMemo(() => {
    ...
    if (pendingForMe.length > 0) {
      result.push({ title: t('expenses.sectionPendingForYou', { count: pendingForMe.length }), data: pendingForMe });
    }
    if (others.length > 0) {
      result.push({ title: t('financial.history'), data: others });
    }
    return result;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [expenses, userId, filter, optimisticReads, readsByExpenseId, t]);
  ...
  async function confirmDelete(expense: Expense) {
    const valor = intl.formatCurrency(expense.amount);
    const statusHint = expense.status === 'approved'
      ? '\n' + t('expenses.deleteHintApproved')
      : expense.status === 'pending'
        ? '\n' + t('expenses.deleteHintPending')
        : '';
    const ok = await confirmDestructive({
      title: t('expenses.deleteTitle', { description: expense.description }),
      warning: t('expenses.deleteWarning', { amount: valor, hint: statusHint }),
      destructiveLabel: t('expenses.remove'),
    });
    if (!ok) return;
  ...
  const renderItem = ({ item }: { item: Expense; section: { title: string } }) => {
    ...
    return (
      <TouchableOpacity
        ...
        accessibilityLabel={`${item.description}, ${intl.formatCurrency(item.amount)}, ${t(statusMeta.labelKey)}${unread ? `, ${t('expenses.unreadA11y')}` : ''}`}
      ...
      >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
              ...
              {item.edit_count > 0 && (
                <View style={{ backgroundColor: 'rgba(107,114,128,0.15)', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 999 }}>
                  <Text style={{ color: '#4B5563', fontSize: 10, fontWeight: '500' }}>{t('expenses.editedBadge')}</Text>
                </View>
              )}
        ...
        {item.status === 'rejected' && item.rejection_reason && (
          <View style={{ marginTop: spacing.sm, padding: spacing.sm, backgroundColor: 'rgba(229,57,53,0.06)', borderRadius: radius.sm }}>
            <Text style={{ fontSize: 11, color: '#E53935' }}>{t('expenses.reasonLine', { reason: item.rejection_reason })}</Text>
          </View>
        )}
        {item.status === 'cancel_pending' && item.cancel_reason && (
          <View style={{ marginTop: spacing.sm, padding: spacing.sm, backgroundColor: 'rgba(232,162,40,0.08)', borderRadius: radius.sm, borderWidth: 1, borderColor: 'rgba(232,162,40,0.3)' }}>
            <Text style={{ fontSize: 11, color: '#B45309', fontWeight: '600' }}>{t('expenses.cancelRequestTitle')}</Text>
            <Text style={{ fontSize: 11, color: '#92400e', marginTop: 2 }}>{t('expenses.reasonLine', { reason: item.cancel_reason })}</Text>
          </View>
        )}
        ...
        {canApprove && (
          <View style={{ flexDirection: 'row', gap: spacing.sm, marginTop: spacing.md }} onStartShouldSetResponder={() => true}>
            <TouchableOpacity
              ...
              accessibilityRole="button"
              accessibilityLabel={t('expenses.rejectExpenseA11y', { description: item.description })}
            ...
            >
              {responding === item.id ? <ActivityIndicator size="small" color={colors.textSecondary} /> : <Text style={{ color: colors.textSecondary, fontSize: font.sizes.sm, fontWeight: font.weights.medium }}>{t('expenseForm.reject')}</Text>}
            </TouchableOpacity>
            <TouchableOpacity
              disabled={responding === item.id}
              onPress={(e) => { e.stopPropagation(); handleDecision(item, 'approved'); }}
              accessibilityRole="button"
              accessibilityLabel={t('expenses.approveExpenseA11y', { description: item.description })}
              accessibilityState={{ disabled: responding === item.id, busy: responding === item.id }}
            ...
            >
              {responding === item.id ? <ActivityIndicator size="small" color="#fff" /> : <Text style={{ color: '#fff', fontSize: font.sizes.sm, fontWeight: font.weights.semibold }}>{t('expensesPage.approve')}</Text>}
            </TouchableOpacity>
          </View>
        )}
        ...
        {canRespondCancel && (
          <TouchableOpacity
            ...
            accessibilityRole="button"
            accessibilityLabel={t('expenses.respondCancelRequest')}
          ...
          >
            <Text style={{ color: '#fff', fontSize: font.sizes.sm, fontWeight: font.weights.semibold }}>
              {t('expenses.respondCancelRequest')}
            </Text>
          </TouchableOpacity>
        )}
        ...
        {expanded && (
          <View style={{ marginTop: spacing.md, paddingTop: spacing.md, borderTopWidth: 1, borderTopColor: colors.borderLight, gap: spacing.sm }} onStartShouldSetResponder={() => true}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', columnGap: spacing.md, rowGap: 2 }}>
                {readers.map(r => (
                  <Text key={r.user_id} style={{ fontSize: 11, color: colors.brand }}>
                    ✓ {t('collab.seen')} · {formatReadAt(r.read_at, t, intl)}
                  </Text>
                ))}
              </View>
            )}
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.xs }}>
              {item.receipt_url && (
                <TouchableOpacity onPress={(e) => { e.stopPropagation(); openReceipt(item); }} accessibilityRole="button" accessibilityLabel={t('expenses.viewReceipt')} style={pillStyle('#374151', '#F3F4F6')}>
                  <Text style={pillText('#374151')}>📎 {t('expenses.receipt')}</Text>
                </TouchableOpacity>
              )}
              {canEdit && (
                <TouchableOpacity onPress={(e) => { e.stopPropagation(); setEditing(item); }} accessibilityRole="button" accessibilityLabel={item.status === 'approved' ? t('expenses.editRevertsA11y') : t('common.edit')} style={pillStyle('#2E7268', 'rgba(46,114,104,0.1)')}>
                  <Text style={pillText('#2E7268')}>✏️ {item.status === 'approved' ? t('expenses.editReverts') : t('common.edit')}</Text>
                </TouchableOpacity>
              )}
              {canCancel && (
                <TouchableOpacity onPress={(e) => { e.stopPropagation(); setCanceling(item); }} accessibilityRole="button" accessibilityLabel={t('expenses.cancelExpense')} style={pillStyle('#B45309', 'rgba(232,162,40,0.1)')}>
                  <Text style={pillText('#B45309')}>🚫 {t('common.cancel')}</Text>
                </TouchableOpacity>
              )}
              {canReopen && (
                <TouchableOpacity onPress={(e) => { e.stopPropagation(); setReopening(item); }} accessibilityRole="button" accessibilityLabel={t('expenses.reopenApprovalA11y')} style={pillStyle('#C07055', 'rgba(192,112,85,0.1)')}>
                  <Text style={pillText('#C07055')}>🔄 {t('expenses.reopen24h')}</Text>
                </TouchableOpacity>
              )}
              {canDelete && (
                <TouchableOpacity onPress={(e) => { e.stopPropagation(); confirmDelete(item); }} accessibilityRole="button" accessibilityLabel={t('expenseForm.deleteExpense')} style={pillStyle('#E53935', 'rgba(229,57,53,0.1)')}>
                  <Text style={pillText('#E53935')}>🗑️ {t('common.delete')}</Text>
                </TouchableOpacity>
              )}
            </View>
    ...
    <View style={{ marginBottom: spacing.lg }}>
          ...
          {unreadCount > 0 && (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              ...
              <Text style={{ fontSize: 11, color: colors.brand, fontWeight: '600' }}>
                {unreadCount === 1 ? t('expenses.newOne') : t('expenses.newMany')}
              </Text>
            </View>
          )}
        </View>
        <Text style={{ fontSize: font.sizes['3xl'], fontWeight: font.weights.bold, color: balance.balance >= 0 ? colors.success : colors.error }}>
          {balance.balance >= 0 ? '+' : ''}{intl.formatCurrency(balance.balance)}
        </Text>
        <Text style={{ fontSize: font.sizes.sm, color: colors.textSecondary, marginTop: 2 }}>
          {balance.balance >= 0 ? t('expenses.youAreOwed') : t('expenses.youOweLabel')}
        </Text>
        <View style={{ flexDirection: 'row', gap: spacing.xl, marginTop: spacing.md }}>
          <View>
            <Text style={{ fontSize: font.sizes.xs, color: colors.textMuted }}>{t('expenses.iPaidApproved')}</Text>
            <Text style={{ fontSize: font.sizes.md, fontWeight: font.weights.medium, color: colors.text }}>{intl.formatCurrency(balance.myTotal)}</Text>
          </View>
          <View>
            <Text style={{ fontSize: font.sizes.xs, color: colors.textMuted }}>{t('expenses.coparentPaid')}</Text>
            <Text style={{ fontSize: font.sizes.md, fontWeight: font.weights.medium, color: colors.text }}>{intl.formatCurrency(balance.otherTotal)}</Text>
          </View>
        </View>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: spacing.xs, paddingTop: spacing.md, paddingBottom: 2 }}>
        {(['all', 'pending', 'approved', 'rejected', 'cancelled'] as const).map(f => {
          const filterLabel = f === 'all' ? t('expenses.filterAll') : f === 'pending' ? t('expensesPage.pendingCount') : f === 'approved' ? t('expenses.filterApproved') : f === 'rejected' ? t('expensesPage.rejectedCount') : t('expenses.filterCancelled');
          return (
            <TouchableOpacity
              key={f}
              onPress={() => setFilter(f)}
              ...
              accessibilityLabel={t('expenses.filterByA11y', { label: filterLabel })}
              style={{
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      {canceling && (
        <ReasonModal
          title={canceling.status === 'approved' ? t('expenses.requestCancelTitle') : t('expenses.cancelExpenseTitle')}
          description={canceling.status === 'approved'
            ? t('expenses.requestCancelDescription')
            : t('expenses.cancelExpenseDescription')}
      ...
      {reopening && (
        <ReasonModal
          title={t('expenses.reopenApprovalTitle')}
          description={t('expenses.reopenApprovalDescription')}
...
function ExpenseAuditPanel({ expenseId }: { expenseId: string }) {
  ...
  if (loading) return <Text style={{ fontSize: 11, color: colors.textMuted, fontStyle: 'italic' }}>{t('expenses.auditLoading')}</Text>;
  if (!rows || rows.length === 0) return <Text style={{ fontSize: 11, color: colors.textMuted, fontStyle: 'italic' }}>{t('expenses.auditEmpty')}</Text>;
        ...
        <View key={row.id} style={{ flexDirection: 'row', gap: 6, marginBottom: 4 }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 11, color: colors.text }}>
              <Text style={{ fontWeight: '600' }}>{ACTION_LABEL_KEYS[row.action] ? t(ACTION_LABEL_KEYS[row.action]) : row.action}</Text>
              <Text style={{ color: colors.textMuted }}> · {intl.formatDateTime(row.at)}</Text>
            </Text>
            {row.reason && (
    ...
    <Modal visible animationType="slide" transparent onRequestClose={onClose}>
      <ModalBackdrop onClose={onClose} align="bottom" dim={0.4} padding={0}>
        <View style={{ backgroundColor: colors.bgElevated, borderTopLeftRadius: radius['2xl'], borderTopRightRadius: radius['2xl'], padding: spacing.xl, paddingBottom: 40, maxHeight: '92%' }}>
          ...
          <Text style={{ fontSize: font.sizes.lg, fontWeight: font.weights.bold, color: colors.text, marginBottom: spacing.md }}>{t('expenses.editExpenseTitle')}</Text>
          ...
          {expense.status === 'approved' && (
            <View style={{ backgroundColor: 'rgba(245,158,11,0.1)', borderWidth: 1, borderColor: 'rgba(245,158,11,0.3)', padding: spacing.md, borderRadius: radius.md, marginBottom: spacing.md }}>
              <Text style={{ fontSize: 12, fontWeight: '700', color: '#B45309' }}>⚠️ {t('expenses.alreadyApprovedTitle')}</Text>
              <Text style={{ fontSize: 11, color: '#92400e', marginTop: 2 }}>{t('expenses.alreadyApprovedHint')}</Text>
            </View>
          )}
          ...
          <ScrollView keyboardShouldPersistTaps="handled">
            ...
            <Text style={labelStyle()}>{t('expenseForm.amount')}</Text>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, paddingVertical: 2 }}>
              {EXPENSE_CATEGORIES.map(c => {
                ...
                <TouchableOpacity
                  ...
                  hitSlop={8}
                >
                  <Text style={{ fontSize: 12, color: category === c.value ? colors.brand : colors.textSecondary }}>{c.icon} {catLabel}</Text>
                </TouchableOpacity>
                );
              })}
            </ScrollView>

            <Text style={labelStyle()}>{t('collab.priorityLabel')}</Text>
            <View style={{ flexDirection: 'row', gap: 6, marginBottom: spacing.md }}>
              {(['info', 'important', 'urgent'] as const).map(p => {
                const prioLabel = p === 'info' ? t('collab.priorityInfo') : p === 'important' ? t('collab.priorityImportant') : t('collab.priorityUrgent');
                return (
                <TouchableOpacity
                  key={p}
    ...
    <Modal visible animationType="slide" transparent onRequestClose={onClose}>
      <ModalBackdrop onClose={onClose} align="bottom" dim={0.4} padding={0}>
        <View style={{ backgroundColor: colors.bgElevated, borderTopLeftRadius: radius['2xl'], borderTopRightRadius: radius['2xl'], padding: spacing.xl, paddingBottom: 40 }}>
          ...
          <Text style={labelStyle()}>{t('expenses.reasonRequired')}</Text>
          <TextInput
            value={reason}
  ...
  return (
    <Modal visible animationType="slide" transparent onRequestClose={onClose}>
      <ModalBackdrop onClose={onClose} align="bottom" dim={0.4} padding={0}>
        <View style={{ backgroundColor: colors.bgElevated, borderTopLeftRadius: radius['2xl'], borderTopRightRadius: radius['2xl'], padding: spacing.xl, paddingBottom: 40 }}>
          <View style={{ width: 36, height: 4, borderRadius: 2, backgroundColor: colors.borderLight, alignSelf: 'center', marginBottom: spacing.lg }} />
          <Text style={{ fontSize: font.sizes.lg, fontWeight: font.weights.bold, color: colors.text, marginBottom: spacing.xs }}>{t('expenses.respondCancelTitle')}</Text>
          <Text style={{ fontSize: font.sizes.xs, color: colors.textSecondary, marginBottom: spacing.md }}>
            {t('expenses.wantsToCancel', { name: expense.paidByName ?? '' })} <Text style={{ fontWeight: '600', color: colors.text }}>{expense.description}</Text> · {intl.formatCurrency(expense.amount)}
          </Text>
          {expense.cancel_reason && (
            <View style={{ backgroundColor: 'rgba(245,158,11,0.1)', borderWidth: 1, borderColor: 'rgba(245,158,11,0.3)', padding: spacing.md, borderRadius: radius.md, marginBottom: spacing.md }}>
              <Text style={{ fontSize: 11, fontWeight: '700', color: '#B45309', textTransform: 'uppercase' }}>{t('expensesPage.reason')}</Text>
          ...
          <View style={{ flexDirection: 'row', gap: spacing.sm, marginBottom: spacing.md }}>
            <TouchableOpacity
              ...
              accessibilityLabel={t('expenses.agreeCancelA11y')}
              style={{
            ...
            >
              <Text style={{ fontSize: 13, fontWeight: '600', color: approved === true ? '#2E7268' : colors.textSecondary }}>✅ {t('expenses.agreeCancel')}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => setApproved(false)}
              accessibilityRole="radio"
              accessibilityState={{ selected: approved === false }}
              accessibilityLabel={t('expenses.disagree')}
              style={{
                flex: 1, paddingVertical: spacing.md, borderRadius: radius.md,
            ...
            >
              <Text style={{ fontSize: 13, fontWeight: '600', color: approved === false ? '#E53935' : colors.textSecondary }}>❌ {t('expenses.disagree')}</Text>
            </TouchableOpacity>
          </View>

          {approved === false && (
            <>
              <Text style={labelStyle()}>{t('expenses.reasonOptional')}</Text>
              <TextInput
                value={reason}
                onChangeText={setReason}
                multiline
                ...
                placeholder={t('expenses.disagreePlaceholder')}
                placeholderTextColor={colors.textMuted}
              />
            </>
          )}
