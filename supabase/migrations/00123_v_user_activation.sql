...
-- Higiene de seguranca (alinha com 00122_secure_views_invoker_and_revoke_anon):
-- security_invoker=true ja garante a RLS das tabelas-base (anon enxerga 0 linhas),
-- mas o GRANT default do schema public deixa SELECT pro papel anon e dispara o
-- advisor anon_exposed. Esta view e leaf/reporting e nunca deve ser lida por anon.
-- Revogar anon limpa o lint sem afetar service_role nem authenticated (que segue
-- protegido pela RLS via security_invoker).
REVOKE ALL ON public.v_user_activation FROM anon;
