...
export default async function AppointmentsPage({
  ...
  // Pending returns (upcoming return_date where no follow-up appointment exists yet)
  const pendingReturns =
    appointments?.filter(
      (a) => a.return_date && a.status === "completed" && a.return_date >= new Date().toISOString().split("T")[0]
    ) || [];
  ...
  const typeConfig: Record<string, { icon: string; label: string; accent: string; dateBg: string }> = {
    rotina: { icon: "🩺", label: "Rotina", accent: "border-l-primary", dateBg: "bg-primary/10 text-primary" },
    emergencia: { icon: "🚨", label: "Emergencia", accent: "border-l-red-400", dateBg: "bg-red-50 text-red-600" },
    retorno: { icon: "🔄", label: "Retorno", accent: "border-l-amber-400", dateBg: "bg-amber-50 text-amber-600" },
    exame: { icon: "🔬", label: "Exame", accent: "border-l-violet-400", dateBg: "bg-violet-50 text-violet-600" },
  };

  function renderAppointmentCard(apt: any, isPast: boolean) {
        ...
        {apt.return_date && (
          <div className="border-t border-gray-100 px-4 py-2 flex items-center gap-2">
            ...
            <p className="text-xs text-muted flex-1">
              Retorno previsto: <span className="font-semibold text-dark">
  ...
  return (
    <div className="max-w-lg mx-auto pb-20">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-bold text-dark">Consultas</h1>
      </div>
      ...
      {pendingReturns.length > 0 && (
        <div className="mb-5 bg-amber-50 border border-amber-200 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-sm font-semibold text-amber-800">Retornos pendentes</h3>
          </div>
      ...
      <section className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-sm font-semibold text-dark">Proximas Consultas</h2>
      ...
      {past.length > 0 && (
        <section className="mb-6">
          <h2 className="text-sm font-semibold text-dark mb-3">Historico</h2>
          <div className="space-y-3">
