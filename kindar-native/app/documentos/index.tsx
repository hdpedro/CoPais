/**
 * Documentos — delega para a pagina PWA /documentos via WebView.
 *
 * A versao nativa anterior tinha lista + upload basico, mas a PWA
 * (DocumentsDashboard + DocumentList + DocumentViewer) e muito mais
 * completa: filtros por categoria/crianca, preview, download, share,
 * analytics de uso. Ao inves de replicar tudo nativo, reusamos a PWA
 * via WebView + /native-bridge.
 */
...
export default function DocumentosScreen() {
  ...
  function openInBrowser() {
    Linking.openURL(`${WEB_URL}/documentos`).catch(() => {
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg, paddingTop: insets.top }}>
      ...
      {sessionPayload ? (
        <>
          <WebView
            ref={webviewRef}
            source={{ uri: `${WEB_URL}/native-bridge?next=${encodeURIComponent('/documentos')}` }}
