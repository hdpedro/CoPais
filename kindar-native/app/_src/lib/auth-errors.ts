 ...
 * (Native).
 *
 * ⚠️ PARIDADE OBRIGATÓRIA com `DEV/src/lib/auth-error-codes.ts` (PWA).
 * Os dois arquivos têm o MESMO conjunto de `AuthErrorCode`, MESMOS mappings
 * e MESMA pt-BR fallback copy. Quando expandir um, expandir o outro no
 * mesmo PR. O teste `tests/unit/auth-errors-parity.test.ts` valida.
 *
 * Por que duplicar em vez de importar do PWA: este projeto é resolvido
 * pelo Metro bundler do Expo, que não consegue cruzar pastas siblings
 * (`DEV/src/` vs `DEV/kindar-native/app/_src/`). Tentar alias quebraria o
 * build EAS (memory `project_kindar_eas_resolver_bug`).
 *
...
export interface MappedAuthError {
  code: AuthErrorCode;
  /**
   * PT-BR fallback usado quando o caller não consegue resolver via i18n.
...
/**
 * Wrapper compat. Caller passa só a `message` (legacy) e recebe a tradução
 * pt-BR como string. Novos callers devem usar `mapSupabaseAuthError` direto
 * pra ter acesso ao `code`.
 *
 * @deprecated Prefira `mapSupabaseAuthError({ message, code }).fallbackMessage`
 *             ou — melhor ainda — branch na UI com `mapped.code`.
 */
export function translateAuthError(message: string | undefined | null): string {
  if (!message) return 'Erro inesperado.';
  return mapSupabaseAuthError({ message }).fallbackMessage;
}
