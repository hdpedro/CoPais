...
export default function WhatsAppScheduleButton({
  ...
  function handleWhatsApp() {
    ...
    const text = t("health.whatsapp.messageTemplate", {
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm space-y-4">
      <h3 className="font-semibold text-dark flex items-center gap-2">
        ...
        {t("health.whatsapp.scheduleViaWhatsApp")}
      </h3>

      <p className="text-sm text-muted">
        {t("health.whatsapp.fillFieldsBelow")}
      </p>

      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-medium text-muted mb-1">
            {t("health.whatsapp.professional")}
          </label>
          <select
      ...
      <div>
        <label className="block text-xs font-medium text-muted mb-1">
          {t("health.whatsapp.reason")}
        </label>
        <input
          type="text"
          value={reason}
          onChange={(e) => setReason(e.target.value)}
          placeholder={t("health.whatsapp.reasonPlaceholder")}
          className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#25D366]/50"
        />
      </div>
