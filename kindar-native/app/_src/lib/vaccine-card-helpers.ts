/**
 * vaccine-card-helpers.ts — helpers client-side para tela carteirinha.
 *
 ...
 *                        include=false pra evitar registrar 2 Influenzas
...
export function canonicalizeVaccineName(name: string): string {
  ...
  const noParens = stripped.replace(/\s*\([^)]*\)\s*/g, ' ').trim().replace(/\s+/g, ' ');
 ...
 *
 * @param parsed Vacina identificada pelo OCR
 * @param existing Lista de vaccination_records existentes do child
 * @param now Data atual (injetável pra testes determinísticos)
 */
...
// Re-export consts útil pra UI consumir sem importar o internal.
