...
- `services/decisions.ts` ← `actions/decisions.ts:{createDecision,castVote,addArgument}` + `api/decisions/route.ts:{POST,PATCH}` (Native create/close) + `api/decisions/vote/route.ts` (Native voto) + tool `create_decision`. **Regra de resolução ÚNICA** `computeDecisionOutcome(members, votes, onIncomplete)` (pura, exportada) usada pelos 3 caminhos: auto-resolução ao votar (`resolveDecisionIfReady`), encerramento manual (`closeDecision` → `expirada` se sem quórum) e a rota nativa de voto. `aprovada` exige TODOS os membros votando `concordo` (um `discordo` veta). Antes a regra estava TRIPLICADA e o `closeDecision` aprovava por maioria dos votos lançados (bug 2026-06-22). Native só delega + lê `status` do banco.
...
- `services/balance-operations.ts` ← `actions/balance-operations.ts:{createBalanceOperation,respondToBalanceOperation}` (PWA) + `api/balance-operations/route.ts:{POST,GET}` (Native create/list) + `api/balance-operations/[id]/route.ts:PATCH` (Native respond). Defesa em duas camadas: (a) trigger DB `derive_custody_balance_direction` (migrations 00102+00103) sempre computa `direction` a partir de `operation_type` no banco — protege binários antigos do Native que mandavam `'to_target'` (não-canônico) violando o CHECK constraint, sem precisar build novo; (b) service centraliza membership gate, mapeamento PG (`23503`/`23514`/`23505`/`42501`/`PGRST116`), side-effects (push `balance_proposal`/`balance_response` + chat icons), `captureServerEvent('balance_operation_created'/'balance_operation_responded')` e race-guard em `respondToBalanceOperation` (UPDATE com `eq('status','pending')` + retorno null → `already_processed`). Native usa `apiFetch` com Bearer; nunca mais escreve direto via `safeWrite`.
- `services/whatsapp-link.ts` ← `actions/whatsapp.ts:{requestWhatsAppLink,verifyWhatsAppOTP}` (PWA) + `api/native/whatsapp/route.ts` (Native `request`/`verify`; Android chama `${WEB_URL}/api/native/whatsapp` — **fix chega por deploy de SERVIDOR, sem EAS/OTA**). Vincular número por OTP: `requestWhatsAppLinkService` **REUSA** a linha dona do `phone_number` (take-over + reativação via UPDATE) em vez de INSERT cego, checa TODO write (nunca reporta "sucesso" sem persistir a pendência) e só envia o OTP DEPOIS de gravar. Rejeita só quando ATIVO+VERIFICADO em outra conta; `code`→HTTP status mapeado no route (400/409/404/502/500). Correto sob o UNIQUE global antigo **e** o índice parcial `WHERE is_active` (00135) — independe da ordem de deploy. 13 testes unit.

- `services/care-routine.ts` ← `actions/care-routine.ts:{saveRoutineGrid,createRoutineOverride}` (PWA) + `api/care-routine/route.ts:POST` (Native `save_grid`/`create_override`) + `api/care-routine/today/route.ts:GET` (Native painel). **Rotina de Leva & Busca** (migrations 00112-00114): grade semanal por `(criança, dia, perna leva/busca)` resolvida no read (`src/lib/care-routine-resolve.ts`, pura, **sem materializar `calendar_occurrences`**); `saveRoutineGrid` faz upsert por célula + delete das que sumiram (**sem clobber**); troca pontual (`createOverride`) com **ciência bilateral** via Foundation collab (`care_routine_override`, migration 00114) + `notifyCollabCreate` non-fatal. Painel **adaptável por `coparenting_groups.arrangement`** (rotating→Herói de Guarda como hoje; together/single→rotina vira herói). `/api/care-routine` está na allowlist do middleware (landmine 3×). Mapeamento PG `23503`/`23514`/`23505`/`42501`/`PGRST116`.
  - **Fase 2** (migration **00115** `care_routine_logs`): `recordRoutineLog` (upsert "Buscou? Sim/Não", op Native `record_log`) + follow-up cron `runCareRoutineFollowUps` (pickup-only +45min, idempotente `channel=followup`) + UI no card. Métrica NEUTRA `src/lib/care-routine-metrics.ts:computeCorresponsibility` (só contagens, premium-gated `tier!=free`). Timeline `src/lib/care-routine-journey.ts:buildChildJourney` + rota `/jornada` (compõe guarda+rotina+atividades). Briefing in-app "🌅 Amanhã" no card.
  - **Fase 3 engine** (migration **00116** `week_parity`): resolver `alternating_week` (`weekParityOf` paridade A/B ancorada seg 2024-01-01) + `custody_based` (`custodyResolver` injetado → responsável da guarda; override vence). custody_based display wired no dashboard+jornada+`getRoutineToday`. **Follow-up (engine pronta+testada):** editor de pattern_type + widget nativo (`docs/care-routine-widget-spec.md`).
...
- `2026-05-29` user Angelino: clicar "Propor ajuste" em "Saldo de dias" no Native disparava `custody_balance_operations_direction_check` violation. Causa: função `directionForType` no native retornava `'to_proposer'`/`'to_target'` (valores inventados), mas o CHECK exige `'proposer_gains' | 'target_gains' | 'neutral' | 'both_zero'`. PWA tinha mapeamento certo desde sempre — Native nunca foi alinhado e em produção a feature simplesmente nunca tinha funcionado (3 ops históricas, todas do PWA). 4º incidente do padrão PWA↔Native (junto com swap proposed_date `2026-05-01`, calendar_occurrences `2026-05-07`, decisions stance `2026-05-18`). **Solução robusta em 2 camadas**: (a) migrations `00102`+`00103` — trigger `derive_custody_balance_direction` BEFORE INSERT que SEMPRE sobrescreve `direction` a partir de `operation_type`. Protege binários antigos em prod sem esperar build EAS — mesmo mandando `'to_target'`, o trigger reescreve antes do CHECK rodar. Padrão "banco como source of truth" das migrations `00074` e `00093`+`00094`. (b) service compartilhado `src/lib/services/balance-operations.ts` (createBalanceOperation/respondToBalanceOperation/listBalanceOperations + mapPgError + race-guard) + API REST `api/balance-operations/route.ts` + `[id]/route.ts`. Native vira thin wrapper com `apiFetch`, deixa de escrever direto via `safeWrite`. PWA action vira thin wrapper. 27 testes unit do service + 7/7 smoke do trigger (simulando bug do Native em prod, ROLLBACK no fim).
- `2026-06-22` 3 bugs reportados (PWA), corrigidos com paridade iOS/Android/PWA (Regra 19):
  - **(1) Decisão "APROVADA" com 1/2 votos** — a regra de resolução estava TRIPLICADA (`resolveDecisionIfReady`, `closeDecision`, `api/decisions/vote`) e SÓ o `closeDecision` (encerramento manual / botão "Encerrar") aprovava por **maioria dos votos lançados**, ignorando quem não votou (não há cron de expiração → veio do "Encerrar"). Solução: extraída `computeDecisionOutcome` (pura, exportada) como fonte ÚNICA usada pelos 3 caminhos — `aprovada` exige TODOS votando `concordo`, encerrar sem quórum → `expirada`, um `discordo` veta. 14 testes (`tests/unit/decision-outcome.test.ts`). Native só delega → paridade automática. `#145`.
  - **(2) Família duplicada com o mesmo e-mail** — `/api/create-group` não tinha guard "já é membro"; a página PWA `/onboarding` redirecionava quem já tem grupo, mas o **endpoint** (ponto ÚNICO de criação PWA+Native) ficava aberto → duplicata via wizard nativo / double-submit. Decisão do dono: **famílias adicionais só por convite** (aceite de convite continua livre). Guard idempotente no endpoint (devolve grupo existente) + na action legada `createGroup` `#145`. Native ganhou guard de UX equivalente no onboarding `#147` — o dado já é protegido pelo servidor, então `#147` é só paridade visual e entra na **próxima versão nativa** (não OTA isolada por causa do release 1.0.22 em curso).
  - **(3) Convidado preso na etapa "Convidar o co-responsável" do quest** — o 2º responsável nunca dispara `markQuestStep('invite_co')` (quem convida é o admin) e `/convite/enviar` é admin-only → travado em 2/5. PWA resolveu no read (`getQuestProgress` marca `invite_co` quando o grupo tem 2+ membros, `#145`). Como o **Native lê `onboarding_quests` crua** (`kindar-native/app/_src/services/quest.ts`), a paridade dos 3 veio por **trigger no banco** (migration `00125_mark_invite_quest_on_group_join`): marca `invite_co` quando o grupo chega a 2+ membros (idempotente) + backfill (43 users; `still_missing=0`) + `REVOKE EXECUTE`. Padrão "banco como fonte de verdade" — paridade nos 3 **SEM OTA** e cura os presos. `#146`.
