...
- **OTA Android** (bugs 10-11, nativo): 2026-06-04, **7 runtimes** (1.0.7→1.0.19) via `npm run ota:android` (`--platform android`, branch `production`). Reporter e **iOS** → recebe no proximo build iOS (**nunca publico OTA pro iOS** — runtime compartilhada). `TARGET_VERSIONS` do script de OTA atualizado pra incluir **1.0.19** (vc36 foi buildado antes destes fixes).
