...
export default function VacinasScreen() {
  ...
  async function handleCreate() {
    ...
    const result = await safeWrite({
      ...
      payload: { group_id: activeGroup.groupId, child_id: selectedChild, vaccine_name: name.trim(), dose_label: dose.trim() || null, administered_date: getBrazilToday(), created_by: userId },
    });
    if (result.success) {
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      setShowForm(false); setName(''); setDose('');
      load();
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      {showForm ? (
        <View style={{ padding: spacing.xl, backgroundColor: colors.bgElevated, borderBottomWidth: 0.5, borderBottomColor: colors.borderLight }}>
          ...
          <TextInput value={dose} onChangeText={setDose} placeholder="Dose (ex: 1a dose, reforco)" placeholderTextColor={colors.textDim} style={{ backgroundColor: colors.bgSurface, borderRadius: radius.md, padding: spacing.md, fontSize: font.sizes.md, color: colors.text, marginBottom: spacing.md }} />
