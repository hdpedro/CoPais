...
// B10 fix — esconder o AIAssistant flutuante em rotas onde a decisão de
// pagamento ou onboarding domina a atenção do usuário. Stripe, Notion e
// Linear escondem chat/FAB nessas surfaces porque ele compete visualmente
// com o CTA primário. Padrão "minimal during conversion".
const HIDE_AI_FAB_ROUTES = ["/assinatura", "/pricing", "/onboarding"];
function shouldHideAIFab(pathname: string | null): boolean {
  ...
  return (
    <>
      ...
      {!isDesktop && (
        <header className="fixed top-0 left-0 right-0 z-40 bg-[#EEECEA]/80 backdrop-blur-2xl px-5 pb-2 flex items-center justify-between safe-area-top">
          ...
          <div className="flex items-center gap-1">
            ...
            {activeGroupId && !hideAIFab && <AIAssistant groupId={activeGroupId} isMobile />}
      ...
      {/* AI Assistant — desktop only (floating), mobile is in header.
          Hidden em rotas de conversão/onboarding (B10) — ver shouldHideAIFab. */}
      {isDesktop && activeGroupId && !hideAIFab && <AIAssistant groupId={activeGroupId} />}
    </>
  );
}
